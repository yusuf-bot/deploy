...
cmd_up() {
  ...
  # Register if not already — cmd_add handles duplicate path detection
  cmd_add "$name" "$type" "$dir"

  # Override port if specified in deploy.yml
  local yml_port="${yml_port:-}"
  if [ -n "$yml_port" ]; then
    local current_port
    current_port=$(get_app_port "$name")
    if [ "$current_port" != "$yml_port" ]; then
      # Update port in registry
      awk -F'	' -v name="$name" -v port="$yml_port" 'BEGIN{OFS="	"} $1==name {$2=port} {print}' "$REGISTRY" > "$REGISTRY.tmp" && mv "$REGISTRY.tmp" "$REGISTRY"
      ok "port for $name set to $yml_port"
    fi
  fi
...
# ── deploy build: build the app from deploy.yml ───────────────────────────────
...
cmd_build() {
  ...
  local name="${yml_name:-}"
  local type="${yml_type:-}"
  local build_cmd="${yml_build:-}"

  [ -z "$name" ] && { err "deploy.yml missing 'name'"; exit 1; }

  # If no build command, infer from type
  if [ -z "$build_cmd" ]; then
    case "$type" in
      node)    build_cmd="npm run build" ;;
      flask)   build_cmd="pip install -r requirements.txt" ;;
      laravel) build_cmd="composer install --no-dev" ;;
      static)  build_cmd="" ;;
      docker)  build_cmd="" ;;
      custom)  build_cmd="" ;;
      *)       build_cmd="" ;;
  ...
  if [ -z "$build_cmd" ]; then
    ok "no build needed for type '$type'"
    return 0
  fi

  cd "$dir" || exit 1
  info "building $name ($type)..."
  info "  cmd: $build_cmd"
  eval "$build_cmd"
  ok "build complete for $name"
}
