...
cmd_list() {
  printf "\n${BOLD}%-15s %-6s %-10s %-40s %-8s${NC}\n" "APP" "PORT" "TYPE" "ENDPOINT" "STATUS"
  echo "──────────────────────────────────────────────────────────────────────────"

  while IFS= read -r line; do
    [[ "$line" =~ ^# || -z "$line" ]] && continue
    local app_name app_port app_type role
    app_name=$(echo "$line" | awk '{print $1}')
    app_port=$(echo "$line" | awk '{print $2}')
    app_type=$(echo "$line" | awk '{print $3}')
    role=$(echo "$line" | awk '{print $4}')
    [ "$role" != "app" ] && continue

    local endpoint="https://${app_name}.${BASE_DOMAIN}"
    local status
    if is_running "$app_name"; then
      status="${GREEN}prod ✓${NC}"
    else
      status="${RED}stopped${NC}"
    fi
    printf "%-15s %-6s %-10s %-40s " "$app_name" "$app_port" "$app_type" "$endpoint"
    echo -e "$status"

    # Check for dev mode
    local dev_enabled
    dev_enabled=$(get_app_field "$app_name" "dev" 2>/dev/null)
    if [ "$dev_enabled" = "true" ]; then
      local dev_port
      dev_port=$(get_app_field "$app_name" "dev_port" 2>/dev/null)
      local dev_endpoint="https://${app_name}.${DEV_DOMAIN}"
      local dev_status
      local dev_pid_file="${PID_DIR}/${app_name}-dev.pid"
      if pid_alive "$(cat "$dev_pid_file" 2>/dev/null)" 2>/dev/null; then
        dev_status="${GREEN}dev ✓${NC}"
      elif lsof -ti :"${dev_port}" &>/dev/null 2>&1; then
        dev_status="${GREEN}dev ✓${NC}"
      else
        dev_status="${DIM}dev off${NC}"
      fi
      printf "%-15s %-6s %-10s %-40s " "  └ dev" "$dev_port" "$app_type" "$dev_endpoint"
      echo -e "$dev_status"
    fi
  done < "$REGISTRY"
  echo ""
...
cmd_add() {
  ...
  # Check if this exact path is already registered (under any name)
  local existing_name
  existing_name=$(awk -F'\t' -v p="$path" '$5==p && $4=="app" {print $1; exit}' "$REGISTRY" 2>/dev/null)
  if [ -n "$existing_name" ]; then
    if [ "$existing_name" = "$name" ]; then
      # Same name, same path — already registered, update type if different
      local existing_type
      existing_type=$(get_type "$name")
      if [ "$existing_type" != "$type" ]; then
        awk -F'\t' -v name="$name" -v type="$type" 'BEGIN{OFS="\t"} $1==name {$3=type} {print}' "$REGISTRY" > "$REGISTRY.tmp" && mv "$REGISTRY.tmp" "$REGISTRY"
        ok "updated type for $name: $existing_type -> $type"
      else
        ok "$name already registered"
      fi
      return 0
    else
      err "path $path is already registered as '$existing_name'"
      err "use: deploy remove $existing_name first, or use the same name"
      exit 1
    fi
  fi

  # Check if name exists with different path
  if grep -q "^${name}\t" "$REGISTRY" 2>/dev/null; then
    err "app '$name' already exists with a different path"
    exit 1
  fi
...
cmd_remove() {
  ...
  awk -F'	' -v name="$name" '$1==name {found=1} END {exit !found}' "$REGISTRY" 2>/dev/null || { err "app '$name' not found"; exit 1; }
  ...
  awk -F'\t' -v name="$name" '$1 != name' "$REGISTRY" > "$REGISTRY.tmp" && mv "$REGISTRY.tmp" "$REGISTRY"
  # Stop dev process if running
  local dev_pid_file="${PID_DIR}/${name}-dev.pid"
  if [ -f "$dev_pid_file" ]; then
    local dev_pid
    dev_pid=$(cat "$dev_pid_file")
    pid_alive "$dev_pid" && kill "$dev_pid" 2>/dev/null
    rm -f "$dev_pid_file"
  fi

  # Docker cleanup
  local app_type
  app_type=$(get_type "$name" 2>/dev/null || true)
  ...
  rm -f "${LOG_DIR}/${name}.dev.log"
  rm -f "${CUSTOM_DIR}/${name}.conf"
  rm -f "${PID_DIR}/${name}.pid"
  rm -rf "${APPS_DIR}/${name}"
  ok "removed $name"

  # Regenerate both prod and dev nginx
  regen_nginx
  regen_dev_nginx 2>/dev/null || true
}
...
cmd_start() {
  ...
  if $daemonize; then
    ...
    health_check "$name" "$port" "$HEALTH_RETRIES" "$HEALTH_INTERVAL" || true
  else
