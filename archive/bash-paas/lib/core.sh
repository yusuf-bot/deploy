...
get_pid() {
  ...
  if [ -n "$pid" ]; then
    ...
    # Verify process name matches expected type
    local name_match=false
    case "$type" in
      docker)  [[ "$proc_name" =~ ^(docker|containerd) ]] && name_match=true ;;
      node|static)  [ "$proc_name" = "node" ] && name_match=true ;;
      flask)  [[ "$proc_name" =~ ^(python|flask) ]] && name_match=true ;;
      laravel)  [ "$proc_name" = "php" ] && name_match=true ;;
      *)  name_match=true ;;
    esac
    if $name_match; then
      save_pid "$name" "$pid"
...
# ── Registry helpers ──────────────────────────────────────────────────────────
get_app_port() { awk -F'\t' -v name="$1" '$1==name && $4=="app" {print $2; exit}' "$REGISTRY"; }
get_type()     { awk -F'\t' -v name="$1" '$1==name && $4=="app" {print $3; exit}' "$REGISTRY"; }
get_path()     { awk -F'\t' -v name="$1" '$1==name && $4=="app" {print $5; exit}' "$REGISTRY"; }
get_app_field() {
  local name=$1 field=$2
  local config_file="$APPS_DIR/${name}/config"
  local line=$(grep "^${field}=" "$config_file" 2>/dev/null | head -1)
  echo "${line#*=}"
}
...
set_app_field() {
  ...
  if grep -q "^${field}=" "$config_file" 2>/dev/null; then
    awk -v field="$field" -v value="$value" 'BEGIN{OFS=""} $0 ~ "^"field"=" {$0=field"="value; found=1} {print}' "$config_file" > "${config_file}.tmp"
...
nginx_listen_block() {
  ...
  local cert="$SSL_CERT"
  local key="$SSL_KEY"
  # Check per-app cert first
  if [ -n "$name" ]; then
    local app_cert
    app_cert=$(get_app_field "$name" "ssl_cert" 2>/dev/null)
    local app_key
    app_key=$(get_app_field "$name" "ssl_key" 2>/dev/null)
    [ -n "$app_cert" ] && cert="$app_cert"
    [ -n "$app_key" ] && key="$app_key"
  fi
...
NGINXEOF
  ...
  while read -r name port; do
    ...
    if [ "$1" != "--reset" ] && [ -f "$custom_file" ]; then
      ...
      default_server_block "$name" "$port" "$ws" >> "$tmpfile"
    fi
  done < <(grep -v '^#' "$REGISTRY" | grep -v '^$' | awk '$4=="app" {print $1, $2}' | sort -u)

  if sudo -n cp "$tmpfile" "$NGINX_CONF" 2>/dev/null; then
    :
  else
    warn "could not write $NGINX_CONF (no passwordless sudo)"
    rm -f "$tmpfile"
    return 1
  fi
  rm -f "$tmpfile"
  # Auto-link prod config
  [ -d /etc/nginx/sites-enabled ] && sudo -n ln -sf "$NGINX_CONF" /etc/nginx/sites-enabled/deploy 2>/dev/null || true
  if sudo -n nginx -t 2>/dev/null; then
    sudo -n nginx -s reload 2>/dev/null && ok "nginx reloaded" || warn "nginx reload failed"
  else
